...
def make_state_from_frame(frame, channel_n, device, hidden_noise=0.1, rng=None):
    ...
    model can break symmetry the way the invisible agents did."""
    x = torch.zeros(channel_n, CANVAS, CANVAS, device=device)
    x[3] = torch.from_numpy(frame).to(device)
    if rng is None:
        x[4:] = torch.rand(channel_n - 4, CANVAS, CANVAS, device=device) * hidden_noise
    else:
        noise = rng.random((channel_n - 4, CANVAS, CANVAS)).astype(np.float32)
        x[4:] = torch.from_numpy(noise).to(device) * hidden_noise
    return x
...
def train(text="SLIME", steps=8000, K=120, channel_n=16, hidden_n=80,
    ...
    frames_np = physarum_frames(K=K, rng_seed=rng_seed, **(sim_kwargs or {}))
