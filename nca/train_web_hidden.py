...
Method: 9-line scaffold that is stored in HIDDEN CHANNEL 4, not visible RGB.
"""
...
def render_word_9_line(text, glyph=12, font_path=FONT_PATH, char_alpha=255, strand_alpha=64):
    ...
    for box in char_boxes:
        draw.text((box["x_pos"], box["y_pos"]), box["char"],
                  font=font, fill=char_color(box["char"]) + (char_alpha,))
...
def train(text, steps=4000, glyph=12, channel_n=16, hidden_n=80,
    ...
    tgt_chars = render_word_9_line(text, glyph, char_alpha=255, strand_alpha=0)
    target_chars = torch.from_numpy(tgt_chars)[None].repeat(batch, 1, 1, 1).to(device)

    tgt_strands = render_word_9_line(text, glyph, char_alpha=0, strand_alpha=255)
    # Use alpha channel of strands as the target for hidden channel 4
    target_hidden = torch.from_numpy(tgt_strands)[None, 3:4].repeat(batch, 1, 1, 1).to(device)


    # Save target image for verification
    if snap_dir:
        Path(snap_dir).mkdir(parents=True, exist_ok=True)
        tgt_img = (tgt_chars.transpose(1, 2, 0) * 255).astype(np.uint8)
    ...
    seed = make_single_seed(text, channel_n, tgt=tgt_chars)
    ...
    for step in range(start_step, steps):
        ...
        x = x.to(device)
        with torch.no_grad():
            loss_rank = F.mse_loss(to_rgba(x), target_chars, reduction="none") \
        ...
        if noise_idx > 0:
            target_noisy = target_chars * (1.0 - noise_idx) + torch.rand_like(target_chars) * noise_idx
            loss_visible = F.mse_loss(to_rgba(x), target_noisy)
        else:
            loss_visible = F.mse_loss(to_rgba(x), target_chars)
            
        loss_hidden = F.mse_loss(x[:, 4:5, :, :], target_hidden)
        loss = loss_visible + loss_hidden * 0.5
        ...
        if step % log_every == 0 or step == steps - 1:
            ...
            print(f"[train_web_hidden_{text}] step {step} vis_loss {loss_visible.item():.5f} hid_loss {loss_hidden.item():.5f} "
...
def grow_word_image(model, text, channel_n, n_steps=120, upscale=8, seed_type="single", device="cpu"):
    with torch.no_grad():
        tgt = render_word_9_line(text, 12, char_alpha=255, strand_alpha=0)
