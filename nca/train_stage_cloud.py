...
def save_word_png(model, text, channel_n, step, snap_dir, x_idx, y_idx, device, target_np, n_steps=60):
    ...
    with torch.no_grad():
        ...
        x_init[:, 4:5, :, :] = x_idx # Initial stage layer
...
def train(text, steps=16000, glyph=12, channel_n=16, hidden_n=80,
    ...
    for step in range(start_step, steps):
        ...
        x_new[:, 4:5] = x_idx # Given target stage
        ...
        target_stage = torch.full_like(x[:, 4:5, :, :], y_idx)
        loss_stage = F.mse_loss(x[:, 4:5, :, :], target_stage)
        
        loss = loss_rgba + loss_stage
        ...
        # Linear progression: advance every 100 steps
...
if __name__ == "__main__":
    ...
    p.add_argument("--snap-dir", default="snaps_stage_cloud")
    a = p.parse_args()
