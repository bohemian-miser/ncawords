...
Method 3: Dilated Envelope Flow (using GaussianBlur).
"""
...
def render_word(text, glyph=12, font_path=FONT_PATH):
    """Whole string on a transparent canvas, per-char color, with a dilated envelope bridge."""
    w, h = word_geometry(text)
    font = ImageFont.truetype(font_path, glyph)
    
    # 1. Render text in full opacity onto a transparent canvas.
    ...
    # 2. Clone this layer and apply heavy blur or dilation.
    halo_img = text_img.copy()
    halo_img = halo_img.filter(ImageFilter.GaussianBlur(radius=3))
    
    # 3. Reduce the opacity of the dilated layer (halo).
    halo_arr = np.array(halo_img).astype(np.float32)
    halo_arr[..., 3] *= 0.25 # Reduce alpha
    halo_img = Image.fromarray(halo_arr.astype(np.uint8))
    
    # 4. Composite the original text on top of the halo.
    combined_img = Image.alpha_composite(halo_img, text_img)
    
    arr = np.asarray(combined_img, dtype=np.float32) / 255.0
    arr[..., :3] *= arr[..., 3:]
