...
def train(emoji="1f642", steps=8000, channel_n=16, hidden_n=96,
    ...
    target_util = float(os.environ.get("NCA_GPU_UTIL", "0.70"))
    if torch.cuda.is_available():
        device = "cuda"
        try:
            torch.cuda.set_per_process_memory_fraction(min(0.70, target_util))
        except Exception:
            pass
        ...
        device = "cpu"
    print(f"Device {device} (target GPU util: {int(target_util*100)}%), starting {label}")
    ...
    # Pure eager mode on CUDA is 40ms/step with zero compile overhead
    pass
    ...
    for step in range(start_step, steps):
        ...
        if step % 200 == 0 or step == steps - 1:
            ...
            if snap_dir:
                ...
                raw_m = getattr(model, "_orig_mod", model); torch.save(raw_m.state_dict(), str(Path(snap_dir) / "latest.pth"))
        ...
        if snap_dir and (step % 200 == 0 or step == steps - 1):
            raw_m = getattr(model, "_orig_mod", model); save_checkpoint(snap_dir, step, raw_m, opt, sched)
