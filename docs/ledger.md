...
#### cheat-ladder (ANALYSIS, 2026-07-24)
Interrogation #3 completes the escalation ladder. Ablation-trained t0 runs: abl-sharedk-r0 = BACKUP-KEEPER (letter-shaped stencil copies in untouched channels, ch4 corr +0.92, regenerated ghost in the ablated channel); abl-sharedk-r1 = DISTRIBUTED STATIC STORE (faint whole-word copy smeared across all channels at amplitudes that evade correlation detectors). Zero hole-completion in both; no active maintenance. THE PRINCIPLE: 'reproduce the t0 input at time T' is a memory task, so memory is the optimal solution — amplifier -> copy-keeper -> backup-keeper are increasingly clever storage, i.e. correct solutions to a mis-specified objective. Development can only be forced by demanding information the input does not contain. Final wave gen-* launched: HOLED scaffold at t0 + loss on the COMPLETE word (+ ablation) — storage caps out, generative completion required. Interrogation battery (backup hunt across all channels, autoscaled grids, total-memory ablation with noise, static-vs-maintained, hole completion, long horizon) awaits any winner. Also: CSE compute lane live (Pi-held SSH sessions, NFS-home venv/checkpoints), running abl-sharedk-r2 replication.
